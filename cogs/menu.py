...
class menu(commands.Cog, name="Créateur", description="Affiche le menu afin de permettre la création, l'édition mais aussi lister les créateurs de tickets."):
    ...
    async def category(self, ctx):
        ...
        embed.add_field(name="1️⃣ - Création", value="Créer un nouveau créateur de billet.", inline=False)
        embed.add_field(name="2️⃣ - Modification", value="Modifier un créateur de billet.", inline=False)
        embed.add_field(name="3️⃣ - Liste", value="Afficher la liste des paramètres d'un billet.",inline=False)
        embed.set_footer(text="Cliquez sur la réaction pour choisir !\n❌ Permet de quitter le menu.")
        q=await ctx.send(embed=embed)
        await q.add_reaction("1️⃣")
        await q.add_reaction("2️⃣")
