...
class fiches(
    ...
    async def check_fiche(self, ctx, member, idS):
    ...
    async def admin_edit(self, ctx, member: discord.Member):
        ...
        chartype = await self.check_fiche(ctx, member, idS)
        if chartype == "ERROR":
        ...
        if chartype != "ERROR":
            ...
            perso = utils.save_data(member, chartype, idS)
            msg, img = await forme(ctx, member, chartype, idS)
            ...
            if reaction.emoji == "1️⃣":
                ...
                await self.edit_fiche(ctx, member, perso, idS, chartype)
            elif reaction.emoji == "2️⃣":
                await q.delete()
                await self.delete_fiche(ctx, member, chartype, idS)
    ...
    async def fiche(self, ctx):
        ...
        if (
            ...
            if chartype != "ERROR":
                ...
                if reaction.emoji == "1️⃣":  # Edition
                    await q.delete()
                    await self.edit(ctx)
                elif reaction.emoji == "2️⃣":  # SUPRESSION
                    await self.delete(ctx)
                elif reaction.emoji == "3️⃣":  # REPRISE
                    await self.reprise(ctx)
                elif reaction.emoji == "4️⃣":  # VOIR
                    await self.see(ctx)
            else:
                await ctx.send("Vous n'avez pas de présentation en cours !")
        else:
            await ctx.send(
                "Impossible de faire une présentation : Les channels ne sont pas configurés !"
            )
    ...
    async def edit_fiche(self, ctx, member, perso, idS, chartype):
        def checkRep(message):
            return (
    ...
    @fiche.command(
        help="Permet d'éditer une fiche",
    )
    async def edit(self, ctx):  # 1️⃣ Du MENU
        member = ctx.message.author
        idS = ctx.guild.id
        emoji = ["1️⃣", "2️⃣", "3️⃣", "4️⃣", "❌"]
        chartype = await self.check_fiche(ctx, member, idS)
    ...
    async def delete_fiche(self, ctx, member, chartype, idS):
    ...
    @fiche.command(help="Suppression d'une fiche en cours.")
    ...
    @fiche.command(help="Permet de reprendre l'écriture d'une fiche.")
    async def reprise(self, ctx):
        member = ctx.message.author
        idS = ctx.guild.id
        chartype = await self.check_fiche(ctx, member, idS)
    ...
    @fiche.command(
        help="Permet de voir une fiche en cours d'écriture", aliases=["check"]
    )
